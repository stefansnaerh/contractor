            ...
            {slice.items?.map((item, index) => {
              ...
              return (
                <PrismicNextLink
                  field={item.service_link}
                  key={index}
                  className=" sm:max-w-[45%] lg:max-w-[30%]"
                >
                  <FadeIn className="flex group flex-col justify-around gap-fluid-32 md:gap-fluid-40  bg-pinkCard hover:bg-orange hover:text-softWhite px-fluid-40 md:px-fluid-48 pt-fluid-40 md:pt-fluid-48 pb-fluid-32 md:pb-fluid-40 rounded-12 shadow-lg transition-all duration-300 ease-in-out h-fill w-fill">
                    <div className="flex justify-between">
                      <h3 className="text-h3 font-headline font-semiBold self-center">
                        {item.service_title}
                      </h3>
